...
const ViewDataComponent = (props: any) => {
  const { onDelete, id, onEdit, editMode } = props;
  return (
    <div className="flex space-x-4 border border-indigo-600 rounded-md justify-between py-4 px-8 mb-4 bg-slate-50 2xl:w-[60rem] md:w-screen">
      <div>
      ...
      <div>
        ...
        <input
          ...
          value={props?.completed === 'true' ? true : false}
      ...
      <div
        className={editMode ? "cursor-not-allowed flex" : "flex cursor-pointer"}
      >
        <svg
          onClick={() => onEdit(props)}
          className={editMode ? "pointer-events-none" : "pointer-events-auto"}
        ...
        <svg
          onClick={() => onDelete(id)}
          className={
            editMode ? "pointer-events-none ml-5" : "pointer-events-auto ml-5"
          }
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
